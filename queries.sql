...
FROM Match_Date md NATURAL JOIN FACTS f
WHERE md.year = '2015' AND f.type = 'possession'
GROUP BY md.month;



-- Tous les matchs et l'equipe qui les ont gagne, ainsi que le nombre de buts (total et par équipe, extérieur ou domicile) --
...
SELECT sq.rank, sq.nbvictoires, t.team_long_name
FROM 
(
SELECT mv.winner, COUNT(*) nbVictoires, DENSE_RANK() OVER (ORDER BY COUNT(*) DESC) rank
FROM match_victoire mv, team t
WHERE t.team_id = mv.winner
GROUP BY mv.winner
) sq JOIN Team t ON t.team_id = winner
